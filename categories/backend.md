...
<div class="container bg-light container-border-radius py-3 px-4 my-lg-4 my-3">
    <div class="posts">
        ...
        <div class="card-columns col-md-12 ">
            {% for backend in site.categories.backend %}
                <div class="card shadow-sm mt-5">
                    <div class="card-header bg-transparent border-0" style="margin-bottom: -44px;">
                        <a href="{{ backend.url | prepend: site.baseurl }}">
                            <img title="{{ backend.title }}" class="card-position img container-border-radius shadow hover-img" src="{{ backend.img-title }}" width="100%">
                        </a>
                    </div>
                    <div class="card-body">
                        <h5 class="text-center"><a title="{{ backend.title }}" href="{{ backend.url }}">{{ backend.title }}</a></h5>
                        {% if backend.tag != null %}
                            <i class="fas fa-tags font-weight-light mr-lg-1">
                                {% for multi-tag in backend.tag %}
